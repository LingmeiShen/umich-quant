...
class MarketFactor:
    ...
    def __init__(self, ticker_, data_):
        ...
        data_[RETURN_COLUMN] = np.log(close / close.shift(1)) #create a new column(return)
        data_ = data_.ix[1:]  # drop first row because it has NaN log return, ix means row, iy column
...
class MarketUniverse:
    ...
    def __repr__(self):
        return 'MarketUniver('+str(self.tickerToMarketFactorDict)+')'
...
class Market:
    ...
    def simulated_returns_dict(self):
        noise=np.random.normal(loc=0,scale=1,size=self.numDays)
        tickerToReturnDict={}
